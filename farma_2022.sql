...
BEGIN
    ...
    VALUES(UUID(), NEW.farma_id, CONCAT(NEW.animal_tag, ' has been updated by', (SELECT CONCAT(first_name, ' ', last_name) FROM farma WHERE farma_id = NEW.farma_id)));
...
BEGIN
    ...
    VALUES(UUID(), OLD.farma_id, CONCAT(OLD.animal_tag, ' has been deleted by ',
        (SELECT CONCAT(first_name, ' ', last_name) FROM farma WHERE farma_id = OLD.farma_id)), OLD.animal_type);
...
BEGIN
    ...
    IF OLD.confirmed = 'N' THEN
        ...
        VALUES ((SELECT mail FROM farma WHERE farma_id = NEW.farma_id),'N',
            ...
            CONCAT(NEW.animal_tag, ' has been confirmed and registered today the ', DATE_FORMAT(NOW(), '%W %M %e %Y')),
            NEW.animal_tag,
            UUID(), 'reminder');
...
(
    id              int auto_increment
        primary key,
    list_of_animals longtext collate utf8mb4_bin null
        check (json_valid(`list_of_animals`)),
...
BEGIN
    ...
    -- statements
    INSERT INTO audit_trail(uuid, user_id, action)
    VALUES(UUID(), NEW.farma_id, CONCAT(NEW.name,' - ', NEW.quantity,' ', NEW.quantity_measure, ' has been added by',
        (SELECT CONCAT(first_name, ' ', last_name) FROM farma WHERE farma_id = NEW.farma_id)));
...
(
    ...
    expiry_timestamp timestamp default current_timestamp() not null on update current_timestamp()
);
...
(
    ...
    production_frequency int                        not null,
    production_period    int                        not null,
    product_start_date   date                       not null,
    product_end_date     date                       not null,
    production_qnty      int                        not null,
    production_measure   int                        null,
    animal_list          varchar(100)               null,
    one_animal           char                       not null,
    farma_id             varchar(60)                not null,
    projection_id        varchar(60) default uuid() null
);
...
(
    ...
    animal_id             int                                   null,
    extract_date          timestamp default current_timestamp() not null on update current_timestamp()
);
...
BEGIN
    ...
    VALUES(UUID(),
           ...
           (SELECT animal_type FROM animal WHERE animal.id = NEW.animal_id));
...
(
    ...
    email_address   varchar(100) charset utf8 null,
    status          char                      not null,
...
BEGIN
    ...
    INSERT INTO audit_trail(uuid, user_id, action, animal_type)
        VALUES(UUID(),(SELECT farma_id FROM animal WHERE id = NEW.animal_id),
...
(
    ...
    farma_id               varchar(120) null,
    qnty_per_cycle         int          null,
    qnty_measure_per_cycle bigint       null
);
...
create  definer = derek@localhost view v_dairy as
select `farma`.`animal`.`id`          AS `id`,
       `farma`.`animal`.`animal_tag`  AS `animal_tag`,
       `farma`.`animal`.`gender`      AS `gender`,
       `farma`.`animal`.`dob`         AS `dob`,
       `farma`.`animal`.`animal_type` AS `animal_type`,
       `farma`.`animal`.`farma_id`    AS `farma_id`
from `farma`.`animal`
where to_days(current_timestamp()) - to_days(`farma`.`animal`.`dob`) > 230;

create  definer = derek@localhost view v_dairy_cow as
select `farma`.`animal`.`id`          AS `id`,
       `farma`.`animal`.`animal_tag`  AS `animal_tag`,
       `farma`.`animal`.`gender`      AS `gender`,
       `farma`.`animal`.`dob`         AS `dob`,
       ...
       `farma`.`animal`.`farma_id`    AS `farma_id`
from `farma`.`animal`
where `farma`.`animal`.`animal_type` = 'cow'
  and to_days(current_timestamp()) - to_days(`farma`.`animal`.`dob`) > 730;

create  definer = derek@localhost view v_meat_cow as
select `farma`.`animal`.`id`          AS `id`,
       `farma`.`animal`.`animal_tag`  AS `animal_tag`,
       `farma`.`animal`.`gender`      AS `gender`,
       `farma`.`animal`.`dob`         AS `dob`,
...
from `farma`.`animal`
where `farma`.`animal`.`animal_type` = 'cow'
  and to_days(current_timestamp()) - to_days(`farma`.`animal`.`dob`) > 30;
...
BEGIN
	...
	UPDATE pwd SET status = 'A' WHERE farma_id = f_id AND status = 'I';
...
BEGIN
	...
	INSERT INTO sick_animals (animal_id,disease_id,reported_date,vet_id,appointment_date, confirmed_id)
	VALUES ( animalID, diseaseID, reportedDate, vetID, appointmentDate, UUID());

	INSERT INTO symptom (disease_id, symptom_name, description)
...
BEGIN
    ...
    IF email = gmail THEN
        ...
        UPDATE pwd SET status = 'I' WHERE farma_id = f_id;
...
BEGIN
    ...
    DECLARE otpid VARCHAR(80);
    DECLARE cur CURSOR FOR SELECT otp_id FROM otp WHERE expiry_timestamp <= NOW() AND status = 'A';
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    OPEN cur;
    read_loop:
    LOOP
        FETCH cur INTO otpid;
        IF done THEN
            LEAVE read_loop;
        END IF;
...
create 
    definer = derek@localhost procedure testss(OUT latest varchar(100))
BEGIN

        SET @VERIFIED = IFNULL((SELECT farma_id FROM farma WHERE farma_id = '5150ba02-55ea-4b66-b7d4-f24b8ae92911'), 'NOTHING');

        IF @VERIFIED  = 'NOTHING' THEN

            SELECT @VERIFIED INTO latest;
            SELECT latest;
...
create  definer = derek@localhost event if not exists evently on schedule
    every '1' DAY
...
create  definer = derek@localhost event if not exists myevent on schedule
    every '30' SECOND
        starts '2023-05-14 09:24:58'
    enable
    do
    BEGIN

        CALL select_expired_otp();
        INSERT INTO test_dbo (name) values ('ewrw');
    END;

create  definer = derek@localhost event if not exists tested on schedule
    every '1' MINUTE
        starts '2023-04-29 16:27:24'
    enable
    do
    BEGIN
    CALL select_expired_otp();
